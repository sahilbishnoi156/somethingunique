...
export default function AccountSetting() {
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      throw new Error(
        data?.message || data?.data?.data || "Failed to delete account"
      );
    } catch (error) {
      console.error(error);
    ...
    <div className="col-span-8 overflow-hidden rounded-xl sm:bg-secondary dark:sm:bg-secondary/30 sm:px-8 sm:shadow py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-1">
        <p className="text-gray-600">
          Not bad! At least it’s not <em>supercoolguy2009@hotmail.com</em>,
          right?
        </p>
      </div>
      <hr className="mt-4 mb-8" />
      {/* Logout Section */}
      <p className="py-2 text-xl font-semibold">Logout Account</p>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-1">
        <p className="my-2">
          Need a break? Hit logout and take a breather. We’ll miss you…
          probably.
        </p>
      ...
      <p className="mt-2">
        Are you sure about this? Once you delete your account, it’s gone. Like,
        <em>really</em> gone. Make sure you’ve backed up anything important.
        We’re talking no “Oops, I didn’t mean to” moments here.
      </p>
